...
#                     self.validate()
                ...
                if (self.step2 % 1000 == 0):
                    self.save_checkpoint(f'bert-v2-{int(self.step2 / 1000)}k-r-final-2.pth.pth', 'loss',
                             loss, show_msg=False)
            ...
            n_epochs += 1
        self.save_checkpoint('bert-v2-15k-r-final-2.pth', 'loss',
                             loss, show_msg=False)
        print(self.step2)
        self.log.close()
...
#             self.write_log('wer', {'dv_' + task: dev_wer[task]})
    ...
    def print_model(self):
        self.model = Bert_Regression(self.feat_dim,
                         **self.config['model'])
