...
from core.bert import Bert_Regression
...
#         self.optimizer = Optimizer(non_frozen_parameters, **self.config['hparas'])
    ...
    def exec(self):
        ...
        for data in self.tt_set:
            name, audio, feat, attention, typeid, feat_len, txt = data
            feat = feat.to(self.device)
            feat_len = feat_len.to(self.device)
            txt = txt.to(self.device)

            with torch.no_grad():    # last_hidden_state = outputs.last_hidden_state
                outputs = self.bert(feat[0], output_attentions=True, output_hidden_states=True)
                # last_hidden_state = outputs.hidden_states[-1]
                last_hidden_state = torch.mean(torch.stack(outputs.hidden_states[-4:]), dim=0)
                 
            hyp = self.model(last_hidden_state)[0]

            an = (((hyp < 0.25) and (txt == 0.0)) or ((hyp >= 0.25) and (hyp < 0.5) and (txt == 0.5)) or ((hyp >= 0.5) and (hyp < 0.75) and (txt == 1.0)) or ((hyp >= 0.75) and (txt == 2.0))).tolist()[0]
            print(name, ' ', hyp.tolist()[0], ' ', txt.tolist()[0], ' ', an)
            names.append(name[0])
            hyps.append(hyp.tolist()[0])
            txts.append(txt.tolist()[0])
            ans.append((((hyp < 0.25) and (txt == 0.0)) or ((hyp >= 0.25) and (hyp < 0.5) and (txt == 0.5)) or ((hyp >= 0.5) and (hyp < 0.75) and (txt == 1.0)) or ((hyp >= 0.75) and (txt == 2.0))).tolist()[0])
