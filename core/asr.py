...
#         linear_fea = torch.squeeze(linear_fea, 1)
#         linear_fea = self.fc(encode_feature[0][-1])
#         out = torch.argmax(torch.softmax(linear_fea, dim=1))

        return out

class Encoder_MMSE(nn.Module):
    """
    ASR model, including Encoder/Decoder(s)
    """
...
class Regression_acoustic(nn.Module):
    """
    Regression_acoustic model
    """

    def __init__(self, input_dim, output_dim):
        super(Regression_acoustic, self).__init__()
