...
from core.data import load_combine_dataset
...
#         self.optimizer = Optimizer(non_frozen_parameters, **self.config['hparas'])
    ...
    def exec(self):
        ...
        for j, data in enumerate(self.tt_set):
            name, audio, feat, feat_len, name2, feat2, feat_len2, txt = data
            feat = feat.to(self.device)
            feat_len = feat_len.to(self.device)
            ...
            hyp = self.model(acoustic_features, linguistic_features)[0]
