...
def visualize_head_average(att_map):
    ax = plt.gca()    
...
def visualize_heads_average(att_map, cols):
    to_shows = []
    att_map = att_map.squeeze()
    average_att_map = att_map.mean(axis=0)
    # Plot the heatmap
    ...
    idx=100
    while True:
        if i < average_att_map.shape[1]:
            j = min(i+idx,average_att_map.shape[1])
            to_shows.append((average_att_map[:,i:j], f'Seg {i}'))
            i += idx
        else:
            break        
...
def visualize_heads_resized(att_map, cols, target_shape):
    to_shows = []
    att_map = att_map.squeeze()
    for i in range(att_map.shape[0]):
        attention_resized = np.resize(att_map[i], target_shape)
        to_shows.append((attention_resized, f'Head {i}'))
    # attention_resized = np.resize(att_map.mean(axis=0), target_shape)
    # to_shows.append((attention_resized, 'Head Average'))
    grid_show(to_shows, cols=cols)
